...
engine = create_engine(URL(
                    account = 'od38843.ap-southeast-1',
                    user = 'asfriendlyascarbon',
                    password = '******',
                    database = 'demo',
                    warehouse = 'demo_wh'))
    ...
    try:
        query = """ SELECT RN, concat(latitude,',',longitude) as LOCATION
                    FROM (SELECT RN
                        ...
                        FROM otodom_data_flatten WHERE rn between 1 and 20000
                        ORDER BY rn  ) """
        ...
        pandas_df.to_sql('otodom_data_flatten_address', con=engine, if_exists='append', index=False, chunksize=1000, method=pd_writer)
